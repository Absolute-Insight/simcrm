...
- **Existing sites must not change behaviour.** `manager_outside_hierarchy` falls back to `All records` in the reader. Note the mechanism, because this plan originally got it wrong: `frappe.db.get_single_value` does **not** return `None` for a never-saved Single — it casts the missing value to the fieldtype's zero value, so a Select yields `""` and a Check yields `0`. `""` is falsy, so `(value or "All records")` still lands on the historical answer. A guard written against `is None` would never fire; that mistake reached Task 4 and had to be reverted. No migration patch anywhere in this plan.
