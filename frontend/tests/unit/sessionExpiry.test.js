...
describe('isSessionGone', () => {
  ...
  it('treats an AuthenticationError as gone', () => {
    expect(
      isSessionGone({ status: 401, exc_type: 'AuthenticationError' }, REP),
    ).toBe(true)
  })
  it('a 403 with the user cookie gone is an expired session', () => {
    expect(
      isSessionGone({ status: 403, exc_type: 'PermissionError' }, GUEST),
    ).toBe(true)
    expect(
      isSessionGone({ status: 403, exc_type: 'PermissionError' }, ''),
    ).toBe(true)
  })
  it('a 403 while still logged in is a real permission refusal', () => {
    expect(
      isSessionGone({ status: 403, exc_type: 'PermissionError' }, REP),
    ).toBe(false)
  })
  it('other failures are not session problems', () => {
    expect(
      isSessionGone({ status: 500, exc_type: 'ValidationError' }, GUEST),
    ).toBe(false)
    expect(
      isSessionGone({ status: 417, exc_type: 'ValidationError' }, REP),
    ).toBe(false)
...
describe('loginUrlFor', () => {
  it('carries the whole location so the person returns where they were', () => {
    const url = loginUrlFor({
      pathname: '/crm/deals/CRM-DEAL-1',
      search: '?view=x',
      hash: '#emails',
    })
    expect(url).toBe(
      '/login?redirect-to=' +
        encodeURIComponent('/crm/deals/CRM-DEAL-1?view=x#emails'),
    )
  })
})
