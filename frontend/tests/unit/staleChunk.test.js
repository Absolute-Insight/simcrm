...
import { isStaleChunkError, reloadOnceForStaleChunk, RELOAD_MARK } from '@/utils/staleChunk'
...
describe('isStaleChunkError', () => {
  it('recognises the browsers\' dynamic import failures', () => {
    ...
    expect(isStaleChunkError({ message: 'error loading dynamically imported module' })).toBe(true)
...
describe('reloadOnceForStaleChunk', () => {
  ...
  it('reloads to the target the first time', () => {
    ...
    expect(reloadOnceForStaleChunk(err, '/crm/deals', { storage, now: 1000, reload })).toBe(true)
  ...
  it('does not loop on a build that is broken for real', () => {
    ...
    expect(reloadOnceForStaleChunk(err, '/crm/deals', { storage, now: 5000, reload })).toBe(false)
  ...
  it('reloads again once the cooldown has passed', () => {
    const reload = vi.fn()
    const storage = memoryStorage({ [RELOAD_MARK]: '1000' })
    expect(reloadOnceForStaleChunk(err, '/crm/deals', { storage, now: 100_000, reload })).toBe(true)
  })

  it('ignores unrelated errors', () => {
    const reload = vi.fn()
    expect(reloadOnceForStaleChunk(new Error('nope'), '/x', { storage: memoryStorage(), now: 1, reload })).toBe(false)
    expect(reload).not.toHaveBeenCalled()
  })
})
