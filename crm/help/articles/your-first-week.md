...
**Start in Suggestions**, not in the deal list. The panel opens from the
sidebar and holds the ranked list of things worth doing today, each with the
reason it appeared. Work down it. Accepting a suggestion creates the task, call
or reply it describes; dismissing one asks why, and that answer is used.
...
7. **Assignment rules** — Settings → Assignment Rules, so new records land on
   somebody.
8. **The assistant** — optional, and off by default. See **The Vectora
   assistant**.
