...
| **Website enrichment** | Automatic rule | Reads an organisation's public website and fills empty fields from it — rules first, and a model only if your administrator switches the fallback on. Values already on the record are kept unless a field is set to always refresh | **Enrich from website** on an organisation |
